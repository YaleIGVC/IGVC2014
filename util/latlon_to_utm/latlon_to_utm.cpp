...
int main()
{
    ...
    ifstream infile("gpslatlon.txt");
    if(infile.is_open()){

        const int MAX_LINE_LEN = 256;
        char line[MAX_LINE_LEN];
        ofstream outfile;
  		outfile.open ("gpsutm.txt");
        while(infile.getline(line, MAX_LINE_LEN)){

        	istringstream liness( line );
        	string latval, lonval;
    		getline( liness, latval, ',' );
    		getline( liness, lonval, ',' );

    		double xcoord, ycoord;

    		UTM(atof(latval.c_str()), atof(lonval.c_str()), &xcoord, &ycoord);

    		std::ostringstream os1;
			os1 << xcoord ;
			std::string xcs = os1.str();

			std::ostringstream os2;
			os2 << ycoord ;
			std::string ycs = os2.str();

    		//std::string xcs = std::to_string(xcoord);
    		//std::string ycs = std::to_string(ycoord);
